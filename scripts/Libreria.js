...
loading2 = {
    ...
    ini: function () {
        alto = $(window).height();
        $('body').append('<div class="loading"><img src="' + loading2.img + '" alt="Cargando" /><div>' + loading2.msj + '</div></div>');
        $('.loading').css("height", alto);
        $('.loading').fadeTo(0, 0.85);
        $('.loading h5').fadeTo(0, 100);
    },
...
msjs = {
    ...
    exitoCargaInsumo: 'La informaci\u00f3n ha sido cargada en el SERUV'
};
...
calen = {
    ...
    validaFecha: { //Validaciones de fechas
        ...
        FechaMIs: function (fecMen, fecMay) {// dd-mm-yyyy
            fec1 = new Date(Date.parse(fecMen));
            fec2 = new Date(Date.parse(fecMay));
            if (fec1 <= fec2) {
                return true;
            } else {
                return false;
...
var NG = { //Navegacion en grids, segun niveles
    ...
    Var: [{ //Variables para pintar grid
        ...
        repinta: null, //Variable de desición
        arreglo: new Array() //guarda datos
    }],
    ...
    repinta: function () {
        ...
        if (NG.Var[NG.Nact].datos.length <= 1) {
            ...
            //---Modificación Jesús Montero Cuevas 19/03/2013-------------
            //agrego un mensaje cuando no existen datos
